...
async def handle_query(data: QueryRequest, db: Session = Depends(get_db)):
    # Run AI Workflow
    response = run_hospital_workflow(data.patient_query)
    ...
    status = "pending"
    if any(keyword in data.patient_query.lower() for keyword in critical_keywords) or intent.strip() == "emergency":
